...
class image_object(object):
    ...
    def __init__(self, imageDirectory, imageName, blockDimension, outputDirectory):
        ...
        # height = vertikal, width = horizontal
        ...
        self.Nb = (self.imageWidth - self.blockDimension + 1) * \
            (self.imageHeight - self.blockDimension + 1)
    ...
    def run(self):
        ...
        print("Computing time :", timestampAfterComputing -
              startTimestamp, "second")
        print("Sorting time   :", timestampAfterSorting -
              timestampAfterComputing, "second")
        print("Analyzing time :", timestampAfterAnalyze -
              timestampAfterSorting, "secon")
        print("Image creation :", timestampAfterImageCreation -
              timestampAfterAnalyze, "second")
        ...
        if self.isThisRGBImage:
            for i in tqdm(range(0, imageWidthOverlap + 1, 1)):
                ...
                for j in range(0, imageHeightOverlap + 1, 1):
                    ...
                    imageBlockGrayscale, imageBlockRGB, i, j, self.blockDimension)
                ...
                for i in range(imageWidthOverlap + 1):
                    for j in range(imageHeightOverlap + 1):
                        imageBlockGrayscale = self.imageData.crop(
                            (i, j, i + self.blockDimension, j + self.blockDimension))
                        imageBlock = Blocks.Blocks(
                            imageBlockGrayscale, None, i, j, self.blockDimension)
                        self.featuresContainer.addBlock(
                            imageBlock.computeBlock())
    ...
    def addDict(self, firstCoordinate, secondCoordinate, pairOffset):
        ...
        if self.offsetDictionary.has_key(pairOffset):
            ...
            self.offsetDictionary[pairOffset] = [
    ...
    def reconstruct(self):
            ...
            for yCordinate in range(2, self.imageWidth - 2):
                if groundtruthImage[xCoordinate, yCordinate] == 255 and \
                    ...
                    if groundtruthImage[xCoordinate - 1, yCordinate] == 0 and \
                        ...
                        linedImage[xCoordinate -
                                   2:xCoordinate, yCordinate, 1] = 255
                        linedImage[xCoordinate, yCordinate -
                                   2:yCordinate, 1] = 255
                        linedImage[xCoordinate - 2:xCoordinate,
                            ...
                            groundtruthImage[xCoordinate, yCordinate - 1] == 0 and \
                            groundtruthImage[xCoordinate + 1, yCordinate - 1] == 0:
                        linedImage[xCoordinate + 1:xCoordinate +
                                   3, yCordinate, 1] = 255
                        linedImage[xCoordinate, yCordinate -
                                   2:yCordinate, 1] = 255
                        linedImage[xCoordinate + 1:xCoordinate +
                                   3, yCordinate - 2:yCordinate, 1] = 255
                    elif groundtruthImage[xCoordinate - 1, yCordinate] == 0 and \
                            groundtruthImage[xCoordinate, yCordinate + 1] == 0 and \
                            groundtruthImage[xCoordinate - 1, yCordinate + 1] == 0:
                        linedImage[xCoordinate -
        ...
        scipy.misc.imsave(self.imageOutputDirectory +
                          timeStamp + "_" + self.imagePath, groundtruthImage)
        scipy.misc.imsave(self.imageOutputDirectory +
                          timeStamp + "_lined_" + self.imagePath, linedImage)
